...
impl Auth for AuthImpl {
    async fn register(
        ...
        println!("Processing Registration username: {:?}", user_name); //registro do nome
        ...
        println!("✅ Successful Registration username: {:?}", user_name); //sucesso no regisitro
        Ok(Response::new(RegisterResponse {}))
    }

    async fn create_authentication_challenge( //criacao do desafio para utilizar o ZKproof
        &self,
        ...
        if let Some(user_info) = user_info_hashmap.get_mut(&user_name) {
            ...
            let auth_id = ZKP::generate_random_string(12);//gera um valor randomico com 12 caracteres
    ...
    async fn verify_authentication(
        ...
        if let Some(user_name) = auth_id_to_user_hashmap.get(&auth_id) {
            ...
            let verification = zkp.verify( //processo de verificacao utilizando zk
                &user_info.r1,
                &user_info.r2,
                &user_info.y1,
                &user_info.y2,
